...
const AIAssistant: React.FC<AIAssistantProps> = ({ resources, members, logs, activeStudent }) => {
  ...
  const chatSession = useMemo(() => getLibraryChat({ resources, members, logs, activeStudent }), [resources, members, logs, activeStudent]);
  ...
  return (
    <div className="max-w-5xl mx-auto h-[750px] bg-white dark:bg-slate-800 rounded-[4rem] border border-white dark:border-slate-700 shadow-3xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-700">
      <div className="p-10 bg-slate-900 text-white flex justify-between items-center shadow-xl">
        <div className="flex items-center space-x-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#84cc16] font-black uppercase tracking-widest">
              {activeStudent ? `${activeStudent.name.split(' ')[0]}'s Study Mode` : 'Registry Intelligence'} • Active
            </p>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-12 space-y-10 custom-scrollbar bg-slate-50/20 dark:bg-slate-900/10">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] ${msg.role === 'user' ? 'bg-[#84cc16] text-white rounded-[2.5rem] rounded-tr-none shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-[2.5rem] rounded-tl-none border border-slate-100 dark:border-slate-700 shadow-xl shadow-slate-200/50'} p-8 text-sm`}>
              {renderContent(msg)}
            </div>
          </div>
        ))}
      ...
      <div className="p-10 border-t border-slate-50 dark:border-slate-700 bg-white dark:bg-slate-800">
        <div className="relative flex items-center">
          <input
            type="text"
            className="w-full bg-slate-100 dark:bg-slate-900 border-none rounded-[2rem] px-8 py-6 outline-none focus:ring-8 focus:ring-[#84cc16]/5 transition-all text-sm font-bold dark:text-white placeholder:text-slate-400"
            placeholder={activeStudent ? "Ask for study advice or explain a topic..." : "Query the registry node..."}
            value={input}
